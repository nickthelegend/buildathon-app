...
import { Menu, Send, MessageSquare, Settings, HelpCircle, Sun, Moon } from 'lucide-react'
...
export default function ChatComponent() {
  ...
  return (
    <div className={`flex h-screen flex-col md:flex-row bg-background text-foreground overflow-hidden ${isDarkTheme ? 'dark' : ''}`}>
      ...
      <aside className="hidden md:flex flex-col w-64 p-4 border-r bg-white dark:bg-gray-800">
      ...
      <main className="flex flex-col flex-1 overflow-hidden">
        ...
        <ScrollArea className="flex-1 p-4 overflow-y-auto" ref={scrollAreaRef}>
          <div className="space-y-4">
            {messages.length === 0 ? (
              // Show Frequently Asked Questions when there are no messages
              <div className="px-4 py-2 border-t bg-muted/50 dark:bg-gray-700">
